...
public class SparshToXornetEmpUploadCsv {
	private static Logger log = Logger.getLogger(SparshToXornetEmpUploadCsv.class);
	...
	private static Map<String, String> empFileMap = new LinkedHashMap<String, String>();

	private SparshToXornetEmpUploadCsv(){
	...
	private static Map<String, String> fileCopyStatus(Connection con, long jobId) throws Exception{
		...
		Map<String, String> filesMap = CopySrcSparshCsv.copyFile(filePrefix, localDir);
		List<String> filesNotToProcess = new ArrayList<String>();
		for(Map.Entry<String, String> entry: filesMap.entrySet()) {
			processFileData(entry, con , jobId, filesNotToProcess);
		}

		ListIterator<String> iterator = filesNotToProcess.listIterator(); 
		while (iterator.hasNext()) { 
			filesMap.remove(iterator.next());
		} 
	...
	private static void processFileData(Entry<String, String> entry, Connection con, long jobId, List<String> filesNotToProcess) throws Exception {
		...
		if(message.equals("NO_FILE_PRESENT")) {
			...
			generateDataMigrationLog(entry, con, jobId, filesNotToProcess, message, localMsg);
		}
	...
	private static void generateDataMigrationLog(Entry<String, String> entry, Connection con, long jobId,
			List<String> filesNotToProcess, String message, String localMsg) throws SQLException, IOException {
		String empFile = entry.getKey();
		try {
